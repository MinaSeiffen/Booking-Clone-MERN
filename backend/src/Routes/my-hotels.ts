...
router.post(
  ...
  [
    ...
    body("adultCount")
      .notEmpty()
      .isNumeric()
      .withMessage("Adult Count Type is required and must be a number"),
    body("childCount")
      .notEmpty()
      .isNumeric()
      .withMessage("Child Count Type is required and must be a number"),
    body("pricePerNight")
      .notEmpty()
      .isNumeric()
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: "There is something went wrong with your request" });
    }
  }
);
...
router.get("/", verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "There is something went wrong in fetching hotels" });
  }
});
...
router.get("/:id", verifyToken, async (req: Request, res: Response) => {
  ...
  try {
    const hotel = await hotelModel.findOne({ _id: id, userId: req.userId });
    res.status(200).json(hotel);
  } catch (error) {
    res
      .status(500)
      .json({ error: "There is something went wrong in finding your hotel" });
  }
});
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "There is something went wrong in Updating Hotel" });
    }
  }
);
